...
check(nonBossExpectedMax < 100, `主線非 Boss 波平均成長受控（最大 +${nonBossExpectedMax.toFixed(0)}% < 100%，排除事件轉場，${SEED_COUNT} seeds 平均）`);
check(bossExpectedMax < 150, `第 5 波後 Boss 波平均成長受控（最大 +${bossExpectedMax.toFixed(0)}% < 150%，${SEED_COUNT} seeds 平均）`);
...
for (const map of Object.values(MAPS)) {
  ...
  const normalTarget = map.id === "plains" ? 19 : 18;
  check(normal >= normalTarget, `【${map.label} / 普通】純塔防撐波數達標（${normal}，目標 ≥${normalTarget}）`);

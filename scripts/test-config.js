...
assert(TOWERS.arcane && TOWERS.arcane.element === "physical" && TOWERS.arcane.range === 130 &&
  ...
  "奧術塔 R47 數值與易傷欄位正確");
assert(TOWERS.beacon && TOWERS.beacon.support === true && TOWERS.beacon.damage === 0 && TOWERS.beacon.fireRate === 0 &&
  TOWERS.beacon.cost === 115 && TOWERS.beacon.slowAura === 0.15 && TOWERS.beacon.reveal === true,
  "引魂燈塔 P0 支援配置正確：無傷害、暴露、15% 減速");
assert(TOWERS.mortar && TOWERS.mortar.element === "fire" && TOWERS.mortar.cost === 160 &&
  TOWERS.mortar.damage === 74 && TOWERS.mortar.fireRate === 0.32 && TOWERS.mortar.splash === 72 &&
  TOWERS.mortar.minRange === 70,
  "墜星臼砲 P0 配置正確：火系高爆、低射速、最短射程");
...
assert(ENEMIES.silencer && ENEMIES.silencer.ability && ENEMIES.silencer.ability.id === "towerMute" &&
  ENEMIES.silencer.ability.range === 115 && ENEMIES.silencer.ability.duration === 2,
  "緘口妖僧 P0 具備 towerMute 配置");
assert(ENEMIES.mirrorling && ENEMIES.mirrorling.ability && ENEMIES.mirrorling.ability.id === "reflectOnce",
  "裂鏡童 P0 具備 reflectOnce 配置");
assert(ENEMIES.warden && ENEMIES.warden.ability && ENEMIES.warden.ability.id === "auraArmor" &&
  ENEMIES.warden.ability.radius === 90 && ENEMIES.warden.ability.damageMul === 0.75,
  "裂界守門人 P0 具備 auraArmor 減傷配置");
...
assert(UPGRADE.costMul >= 1.5 && UPGRADE.costMul <= 1.6, `Lv.10 造價曲線維持後期金錢出口（costMul=${UPGRADE.costMul}）`);
{
  ...
  const regularAttackRanges = Object.values(TOWERS).filter((t) => !t.support && t.id !== "mortar").map((t) => t.range);
  assert(Math.min(...regularAttackRanges) >= 120 && Math.max(...regularAttackRanges) <= 140 && TOWERS.mortar.range === 170 && TOWERS.mortar.minRange > 0,
    `一般攻擊塔未全圖化，臼砲用外環補例外（一般 ${Math.min(...regularAttackRanges)}~${Math.max(...regularAttackRanges)}px，mortar ${TOWERS.mortar.range}/${TOWERS.mortar.minRange}）`);
  assert(TOWERS.support.range === 150, "聖光塔支援射程提高到 150px");
  assert(TOWERS.beacon.range === 145, "引魂燈塔支援射程為 145px");
}
...
for (const e of Object.values(EVENT_WAVES)) {
  ...
  if (e.special && (!e.special.type || !ENEMIES[e.special.type] || !e.special.role)) badEv++;
...
assert(EVENT_WAVES.eclipse && EVENT_WAVES.eclipse.towerDamageMul === 0.85 && EVENT_WAVES.eclipse.goldMul === 1.4,
  "蝕火波 P0 會降低塔傷並提高擊殺金");
assert(EVENT_WAVES.pilgrim && EVENT_WAVES.pilgrim.special && EVENT_WAVES.pilgrim.special.role === "pilgrim" &&
  EVENT_WAVES.pilgrim.special.rewardMul === 8,
  "朝聖波 P0 具備事件專用高獎勵朝聖者");
